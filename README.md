...
`clean` 正式样本。温度图例只显示实际测量窗口平均温度，不显示设定温度。

所有分析图的图例均位于画框右侧。作图单元只显示曲线和图例，不输出原始拟合数值、公式或
结果表；需要审计这些数值时，在可选导出的 `selection_manifest.json` 中查看。
Notebook 图采用作用域隔离的通用论文绘图样式：锁相幅值和相位使用共享横轴的上下双面板，
温度/频率曲线同时用颜色、点型和线型区分，图例注明误差棒类型。可选导出统一生成 600 dpi
PNG、PDF 和可编辑 SVG；该样式是通用起点，并不宣称符合某一本期刊的具体投稿规范。
