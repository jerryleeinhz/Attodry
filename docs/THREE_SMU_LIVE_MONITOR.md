...
`three_smu_cli monitor-live` 是三台 Keithley 2400 的终端状态面板。它不依赖 Lock-in、
...
默认从 ignored 的 `config/hardware.local.toml` 读取三台语义角色和安全限制，每秒显示一帧；
...
面板的每一帧顺序读取 `smu_bias`、`gate_top`、`gate_bottom`，包含：
...
此选项每一帧、每台 SMU 最多消费一个 queue 项。非零/未知响应只会显示 warning；监控不会清除
