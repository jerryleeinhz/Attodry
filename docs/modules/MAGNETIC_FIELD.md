...
2. 连接初始化状态只接受严格的 0/1。连接后读取完整状态，拒绝非零 error、超限
   ...
   满足 3 T invariant。否则不发送 toggle。toggle 后第一条 acknowledgement readback 的
...
- X/Z 命名、3 T resultant invariant、read-before-toggle、状态/readback、异常 zero 和
