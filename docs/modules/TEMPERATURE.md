...
2026-09-22 安全顺序更新：新的 `set_temperature_and_enable(target_k)` 先写入并
确认目标，再开启温控；OFF→ON 后仍强制复写目标。四个温度运行入口统一使用此
路径，避免基温时启用已保存的 300 K 旧目标。下文 control-before-setpoint
描述属于历史验收记录；当前顺序是在其前面增加安全预写，不删除后置复写。
96 项相关 fake-DLL/VISA 回归通过，完整目标端离线回归 588 项通过。
当日低温实机结果见 PROJECT_HANDOFF 的当前阶段，不能由离线通过推定成功。
