...
""",
    ...
    def temperature_scan_config(
        ...
        text = Path("config/hardware.example.toml").read_text(encoding="utf-8")
        # The merged example uses segments, not the commented legacy fields.
        scan_start = text.index("[temperature_scan]")
        scan_end = text.index("\n[", scan_start + 1)
        text = text[:scan_start] + (
            "[temperature_scan]\n"
            f"start_k = {start_k}\nstop_k = {stop_k}\nstep_k = {step_k}\n"
            'run_name = "fake-temperature-scan"\nnote = "offline test"\n'
